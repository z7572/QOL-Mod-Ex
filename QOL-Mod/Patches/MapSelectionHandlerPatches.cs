...
public static class MapSelectionHandlerPatches
{
    ...
    [HarmonyPostfix]
    [HarmonyPatch(typeof(MapSelectionHandler), "ListenForControllerInput")]
    private static void ListenForControllerInputPostfix(MapSelectionHandler __instance)
    {
        if (__instance.Active)
        {
            if (Input.GetKeyDown(KeyCode.Q) || Input.GetKeyDown(KeyCode.LeftArrow))
            {
                var onLeftArrowClickedMethod = AccessTools.Method(typeof(MapSelectionHandler), "OnLeftArrowClicked");
                onLeftArrowClickedMethod.Invoke(__instance, null);
            }
            else if (Input.GetKeyDown(KeyCode.E) || Input.GetKeyDown(KeyCode.RightArrow))
            {
                var onRightArrowClickedMethod = AccessTools.Method(typeof(MapSelectionHandler), "OnRightArrowClicked");
                onRightArrowClickedMethod.Invoke(__instance, null);
            }
        }
    }
